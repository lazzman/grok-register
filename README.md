...
| `proxy` | 注册和 OAuth 请求使用的 HTTP(S) 代理；支持 `http://host:port` 和 `http://user:password@host:port`，凭据中的特殊字符需使用 URL 百分号编码。可在用户名等位置写入 `{id}`，配合 `sticky_proxy` 使用。Camoufox（Firefox）不能直接发送代理用户名/密码，启动时会在本机 `127.0.0.1` 做认证转发 |
